...
class TestHotel:
    def test_unlock_door(self):
        ...
        3.断言返回done
        """
        comm_id = kafka_device_command.unlock_hotel_no_wait()
        time.sleep(3)
        try:
    ...
    def test_lock_door(self):
        ...
        3.断言返回done
        """
        comm_id = kafka_device_command.lock_hotel_no_wait()
        time.sleep(3)
        try:
            assert km.is_conform_to_storage_command_msg(comm_id=comm_id, target_value='running') is True
            assert km.is_conform_to_storage_command_msg(comm_id=comm_id, target_value='done') is True
    ...
    def test_release_hotel(self):
        """
        1.发送释放指令
        2.断言返回running
        3.断言返回done
        """
        comm_id = kafka_device_command.release_hotel_no_wait()
        time.sleep(3)
        try:
            assert km.is_conform_to_storage_command_msg(comm_id=comm_id, target_value='running') is True
            assert km.is_conform_to_storage_command_msg(comm_id=comm_id, target_value='done') is True
...
if __name__ == '__main__':
    pytest.main(['-s',
                 ...
                 '--html=reports/auto_test_report1.html',
                 '--alluredir=alluredir/'
                 ])
